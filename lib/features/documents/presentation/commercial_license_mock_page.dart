...
class _CommercialLicenseMockPageState
    extends State<CommercialLicenseMockPage> {
  ...
  Future<_MockDoc> _load() async {
    final raw = await rootBundle
        .loadString('assets/mock/commercial_license.json');
...
void _openShareQr(String link) {
  ...
  showModalBottomSheet(
    context: context,
    isScrollControlled: true, // allow taller content & dragging
      ...
      return SafeArea(
        ...
        child: LayoutBuilder(
          builder: (ctx, constraints) {
            ...
            // Base QR size by width, then cap by available height
            final qrMaxByWidth = (w - 32).clamp(160.0, 340.0);
            // Leave room for title, handle, text, button (~230px)
      ...
      body: FutureBuilder<_MockDoc>(
        ...
        builder: (context, s) {
          ...
          return ListView(
            ...
            children: [
              // — Improved header (compact horizontal layout) — //
              Container(
                decoration: BoxDecoration(
              ...
              // — Certificate as a physical card — //
              Container(
                decoration: BoxDecoration(
                  color: Colors.white,
...
// ── Reusable UI atoms ────────────────────────────────────────────────────────
class _VerifiedCrest extends StatelessWidget {
  const _VerifiedCrest();
  @override
  Widget build(BuildContext context) {
    return Container(
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        border: Border(
            bottom: BorderSide(
                color: Colors.white.withOpacity(0.07), width: 0.6)),
      ),
      child: Row(
        children: [
          SizedBox(
